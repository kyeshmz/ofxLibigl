  ...
  bool update_matrices)
{
  ...
  if(update_matrices)
  {
    ...
    if (orthographic)
    {
      ...
      float h = tan(camera_view_angle/360.0 * 3.1415926535897932384626433832795) * (length);
      ortho(-h*width/height, h*width/height, -h, h, camera_dnear, camera_dfar,proj);
    }
    else
    {
      float fH = tan(camera_view_angle / 360.0 * 3.1415926535897932384626433832795) * camera_dnear;
